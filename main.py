...
image_path = "C:\\Users\\annap\\PycharmProjects\\pythonProject\\Screenshot 2023-11-06 152852.png"
...
def LinearContrast2(image, new_min=0, new_max=400):
    min_value = np.min(image)
    max_value = np.max(image)
    output_image = np.clip(((image - min_value) / (max_value - min_value)) * (new_max - new_min) + new_min, 0,
                           255).astype(np.uint8)
    return output_image
...
image_path = ("C:\\Users\\annap\\PycharmProjects\\pythonProject\\Screenshot 2023-11-06 112227.png")
